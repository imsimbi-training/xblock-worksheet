...
function WorksheetBlock(runtime, element) {
  ...
  function saveField(event) {
    ...
    pre.text(text || placeholder);
    if (text) inputContainer.addClass("value");
    else inputContainer.removeClass("value");
    setDirty(true);
  }
  ...
  $(".input", element)
    ...
    .remove();
  $(".input", element).on("click", editField);
  var numRepeats = $(".repeat").length;
  if (numRepeats > 0) {
    var tdAddButton = $(
      '<button class="add"><i class="fa fa-2x fa-plus"></i></button>',
    );
    var tdDeleteButton = $(
      '<button class="delete"><i class="fa fa-2x fa-trash-o"></i></button>',
    );
